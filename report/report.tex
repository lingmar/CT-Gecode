...
\begin{abstract}
  Within constraint programming, which is a programming paradigm for solving 
  ...
  is to implement the reasoning for a particular constraint, and Gecode~\cite{Gecode},
  which is an open source constraint programming
  solver written in C++ with state-of-the-art performance, presently has two propagation 
...
\begin{abstract}
  ...
  indispensable contributions to this work, in a particular order:
  ...
  seems to believe more in student's potentials than they do themselves.

  % Christian
  % - Utvecklat Gecode
  % - Gett mig vägledning, tips och råd när jag fastnat
  Christian Schulte, who gave me useful pieces of advise in an early stage
  ...
  comments this report and some much-needed encouragements.
...
\begin{definition}
  ...
  \begin{itemize}
    \item   A \emph{constraint}~$c$ on~$V$ is a relation, 
      denoted by~$\mathit{rel}(c)$. The associated variables~$V$ are denoted~$\mathit{vars}(c)$,
      and we call~$|\mathit{vars}(c)|$ the \emph{arity} of~$c$. The relation
      $\mathit{rel}(c)$ contains the set of~$n$-tuples that are allowed
    ...
    \item For a constraint~$c$ on $V$, the~$n$-tuple~$\tau$ is 
      ...
      $\tau$ is a member of~$\mathit{rel}(c)$.
    \item For an~$n$-ary constraint~$c$, involving a variable~$v$ such that
      the value~$a \in \Dom{v}$, the~$n$-tuple~$\tau$ is a 
      \emph{support for}~$(v,a)$ on~$c$ if and only if~$\tau$ is a support on~$c$
      and~$\tau[v] = a$.
      If such a tuple~$\tau$ exists,~$(v,a)$ is said to have a support on~$c$.
    \end{itemize}
\end{definition}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item Let~$c$ be an $m$-ary constraint on a subsequence~$V_m$ of~$V$,
      where~$m \leq n$.
      A store~$s$ is an \emph{assignment store} for~$c$ if all variables
      in~$V_m$ are fixed by~$s$.
...
\begin{definition}
  ...
  constraint such that~$\mathit{rel}(c)$ is defined explicitly by listing all the
...
that stores which tuples from the input table that are currently valid.
...
For such a trail-based solver, reversing a reversible sparse-bit set
...
\begin{itemize}
  ...
  \item {\Bitset}s have a denser representation of the active bits than reversible sparse
    bit-sets have -- the non-zero words are compressed to lay contiguously in memory.
\end{itemize}
...
that are allocated in a memory area specific to the current node,
...
data structures and try to decrease the levels of indirection, 
...
using a tool~\cite{xcsp2mzn} for translating XCSP2.1 models
into MiniZinc.
...
\begin{algorithmic}[1]
  ...
  \WHILE[executed in the propagator proper]{$W_R\neq\emptyset \lor W_S\neq\emptyset$}
    ...
    \FOREACH{$\langle x,n \rangle \in W_S$ where $n \in \Dom{x}$}
      \STATE $\FindSupport(\langle x,n \rangle)$
    \ENDFOREACH
...
and CT, B, and I all ran out of memory on~$36$ of them.
