...
which have been modified since last time. Propagators that use
...
signature for posting CT the propagator is equal to the signature for the existing
propagators. The tuple set
is only used upon initialisation of the fields, except for the variant CT($T$) where
the tuple set is maintained as a field.

CT supports both integer domains and boolean domains, controlled with C++ templates
...
and also a lot of time is spent in \FilterDomains().
...
\Algoref{algo:incremental} shows propagation algorithm for the incremental
tuple set propagator. When the propagator is beint posted,
...
\FindSupport($\Tuple{x,n}$) for all literals~$\Tuple{x,n}$ whose
...
\caption{Clear support data unsupported literal $\langle x,n
...
listed in~\ref{tab:benchmarks}. The propagators are
denoted:
