...
compared to when enforcing value- or bounds consistency, but might be more costly.
...
is a sparse-set~\cite{Briggs93anefficient,Schaus13sparse-setsfor}
...
even when the bit-set is sparse; hence the name.

From hereon, we call the datastructure representing integers
\emph{int}.
...
upon backtrack. In the following sections, a data structure that is 
like a reversible sparse bit-sets except that it is not reversible
is called a sparse bit-set.
...
By the notation~$0^{64}$ we mean a $64$-bit computer word that has all its
bits set to~$0$.
...
\begin{itemize}
  \item \Words~is an array of~$p$ 64-bit words, where each word
    (abstractly) maps to an element in the set of 
    words~$\Set{w_0, w_1, \dots, w_{p-1}}$.
    Initially, $\Words[i] = w_i$~for all~$i$.
    This array defines the current
...
\begin{itemize}
  ...
  \item intersectWithMask() in~\linesref{line:intersect:1}{line:intersect:9}
    ...
    can actually skip Line 31 and 33 because it is unnecessary
    to save information about the zerowords in a copy-based solver such as Gecode.
    We keep this
...
\begin{itemize}
  ...
  \item For~$\Residues$, maintain as invariant that $\Residues[x,a]$~is the
    \emph{highest} index in \Words~where a support for~$(x,a)$ can be found,
    if such an index exists. This property holds upon initalisation, since
    $\Residues[x,a]$ will be set to the latest found index of a support for~$(x,a)$
    in line~$21$ in \InitialiseCT(). The invariant is maintained by executing the
    loop in intersectIndex() from highest index to lowest index instead of 
    the other way around. 
    By letting intersectIndex() take an extra argument that defines the loop
    limit, we can skip a few iterations by starting the loop at
    index~$\Residues[x,a]$, by passing $\Residues[x,a]$ as an argument to
    intersectIndex() in \FilterDomains().
...
and the variant is denoted CT($B$) in the
