...
\newcommand{\Definition}[1]{\paragraph{{Definition.}~{{\emph{#1}}}}}
...
\begin{abstract}
  In constraint programming, which is a programming paradigm for solving
  combinatorial (optimisation) problems,
  relations among variables are expressed using constraints ---
  one such useful constraint is \Table, which
  expresses the possible combinations that the variables can take
  as a set of tuples. All constraints have a propagation algorithm, whose purpose
  is to implement the reasoning for a particular constraint. Gecode~\cite{Gecode},
  which is an open-source constraint-programming
  solver written in C++ with state-of-the-art performance, currently has two propagation
  algorithms for the \Table~constraint, and one for a closely related constraint
  called~\Regular.
  In this bachelor dissertation, a new propagation algorithm for
  the \Table~constraint, called
  compact-table, first proposed in~\cite{\CTpaper}, is for the first time
  designed and optimised for, and implemented and evaluated in Gecode.
  The implementation is evaluated on a large variety of benchmarks, and the
  results indicate that compact-table outperforms the existing propagators
  for~\Table~in Gecode, as well as the propagator for the~\Regular~constraint.
...
\begin{abstract}
  I wish to express my gratitude towards the following people, and state their
  indispensable contributions to this work:

  % Pappa
  % - Blixtsnabb feedback
  % - Tålmodig
  % - Engagerad
  % Tvåfaldig guldmedaljör i engegemang och snabbhet i feedback
  My supervisor Mats Carlsson, without whom I would --- in several respects ---
  never have conducted this work, and who,
  if supervising were an olympic sport, would be at least a
  twofold gold medalist in dedication and feedback loop speed,
  and will probably have a weekend free from work any year soon.

  % Pierre
  % - inspirerande lärare
  % - höga krav -> blir bättre (hög tanke om studenter, nöjer sig inte
  %   med halvdåligt, inte tappat hoppet om studenter)
  % Pierre Flener, reviewer of this work
  Pierre Flener, reviewer of this work, who has so far kept me busy for
  at least $533.33$ hours, or~$20$ credits,
  with inspiring lectures and assignments, during which
  I achieved a large amount of the prerequisite for this work,
  including Oxford English grammar rules and what to say and what not
  to say about NP-completeness in a bar (serving chocolate milk), and who
  seems to believe more in students' potentials than they do themselves.

  % Christian
  % - Utvecklat Gecode
  % - Gett mig vägledning, tips och råd när jag fastnat
  Christian Schulte, who gave me useful pieces of advice in an early stage
  of the work and who has kindly answered my questions and helped me with
  issues throughout the process.

  % Johan
  % - Intressanta diskussioner
  % - Profilerat och optimerat
  % - Läst min rapport
  % - Bra på C++
  Johan {\"O}fverstedt, with whom I have had many interesting discussions, and who
  increased my skills of profiling and optimising C++ code, and provided useful
  comments on this report and some much-needed encouragement.

  % Albin
  % - Gett feedback på min rapport
  % - Sagt till mig när jag ska gå hem
  Albin Stjerna, who has carefully read every character of this report not only
  once but~$n$ times, where~$n$ is an integer larger than~$3$,
  and whose reminders of when to stop working increased the quality
  of this work by a factor~$q$, and my well-being with a factor~$w$, where~$q$
  and~$w$ are constants larger than~$1$.
  More experiments are needed to determine these values with a larger accuracy.
...
\begin{definition}[\textbf{Constraints}]
  ...
  \item   A \emph{constraint}~$c$ on a subsequence of~$V$
    ...
    \item Let~$\tau$ be an~$r$-tuple of values associated with an
      ...
      $\tau$ exists, then~$(v,a)$ is said to have a support on~$c$.
    \end{itemize}
\end{definition}
...
\begin{definition}[\textbf{Stores}]
  A \emph{store}~$s$ is a function, mapping a finite sequence of
...
\begin{definition}[\textbf{Propagators}] \label{def:prop}
  A \emph{propagator}~$p$ is a function mapping stores to stores:
  \begin{equation*}
    p: store \to store
  \end{equation*}
...
\begin{definition}[\textbf{Range consistency}]
...
\begin{definition}[\textbf{Bounds consistency}]
   A constraint~$c$ is \emph{bounds consistent} on a store~$s$
...
\begin{definition}[\textbf{Domain consistency}]
 A constraint~$c$ is \emph{domain consistent}
...
\begin{definition}[\textbf{Table constraints}]
 A (positive\footnote{There are also negative table constraints that list the
...
called~\Constraint{Regular}, which constrains a sequence of variables
...
OR-tools (Google Optimization Tools)~\cite{ORtools},
...
% as there are typically less ranges than individual values, iteration over ranges can be more efficient.
...
An instance was filtered out if it i) could be solved
within $1$~s for all propagators (as there is too much
measurements noise on easier instances),
or ii) caused a memory-out for at least one of the propagators
(as the memory limit varies between different hardware and
software platforms).
  ...
  \item[B --- Basic positive tuple set propagator]
    This is based on~\cite{DBLP:journals/ai/BessiereRYZ05}.
     ...
     \begin{algorithm}
       ...
       \label{algo:basic}
       \begin{algorithmic}[1]
         \PROCEDURE $\Extensional(s:~\text{store}) :~\text{store}$
           \FOREACH{$x \in X$}
             \STATE $S[x] \gets \bot$
           \ENDFOREACH
           \FOREACH{$x \in X$}
             ...
             \FOREACH{$n \in \Dom{x}$}
               \IF{$S[x] = \bot$}
                 \STATE $\ell \gets L[x,n]$
                 \WHILE{$\ell\neq\bot \land \exists y \in X : \ell[y] \not\in \Dom{y}$}
                   \STATE $\ell \gets L[x,n] \gets$ next tuple for $\langle x,n \rangle$
                 \ENDWHILE
                 \IF{$\ell = \bot$}
               ...
               \ENDIF
             \ENDFOREACH
             \STATE $s \leftarrow s[x \mapsto \Dom{x} \setminus \Set{N}]$
             \IF{$\Dom{x} = \emptyset$}
               \RETURN $\emptyset$
             \ENDIF
           \ENDFOREACH
...
\begin{algorithmic}[1]
  ...
  \ELSE[executed in an advisor]
    ...
    \ENDFOREACH
  \ENDIF
  \WHILE[executed during propagation]{$W_\text{R}\neq\emptyset \lor W_\text{S}\neq\emptyset$}
    \FOREACH{$\langle x,n \rangle \in W_\text{R}$}
      \STATE $s \leftarrow s[x \mapsto \Dom{x} \setminus \Set{n}]$
      \IF{$\Dom{x} = \emptyset$}
        \RETURN $\emptyset$
      \ENDIF
    \ENDFOREACH
    \STATE $W_\text{R} \gets \emptyset$
    \FOREACH{$\langle x,n \rangle \in W_\text{S}$ where $n \in \Dom{x}$}
      \STATE $\FindSupport(\langle x,n \rangle)$
    \ENDFOREACH
    \STATE $W_\text{S} \gets \emptyset$
  \ENDWHILE
  \RETURN $s$
\end{algorithmic}
\end{algorithm}
...
If no such tuple exists, then the literal is added to~$W_\text{R}$, else the tuple is added
...
\begin{algorithmic}[1]
  ...
  \ENDWHILE
  \IF{$\ell=\bot$}
    \STATE $W_\text{R} \gets W_\text{R} \cup \{ \langle x,n \rangle\}$
  \ELSE
    \FOREACH{$y \in X$}
...
runtime, including parsing the model file and the posting
the propagators.
...
%  LocalWords:  bg cp gecode sbs modelling LeCompte th ary CSP pre eq
%  LocalWords:  currTable algo initSparseBitSet initialises nbits MSB
%  LocalWords:  isEmpty clearMask flipMask addToMask intersectIndex
%  LocalWords:  intersectWithMask ints nsupports updateTable thm XCSP
%  LocalWords:  filterDomains currtable validcond updateTableDelta JC
%  LocalWords:  propagateFix indexOfFixed generalise lcccc runtimes
%  LocalWords:  MiniZinc Xeon DFA findsupport removesupport BDD MDD
%  LocalWords:  Rands Dubois Langford Nonograms generalised astra
%  LocalWords:  generalisations mybib Quat
