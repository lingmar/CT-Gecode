...
Implementing a propagator in Gecode means implementing the following parts:

\begin{description}
  \item[Posting.] Posting a propagator requires a \emph{constraint post function},
...
\begin{itemize}
  ...
  \item In the variant using delta information, the current implementation
    uses the incremental update if~$\Delta_x < \Dom{x}$. It is possible
    that this condition can be generalised to~$\Delta_x < k \cdot \Dom{x}$,
