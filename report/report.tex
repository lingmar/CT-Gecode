...
\begin{definition}
  ...
  respective variable. For a variable~$x_i \in X$, its domain~$D_i$ is denoted 
  by~$dom(x_i)$.
  \begin{itemize}
    \item   A \emph{constraint} on~$X$ is a relation, 
      ...
      for~$X$. 
    \item   For an~$n$-tuple~$\tau = (a_1,\ldots,a_n)$ associated with~$X$, we
    ...
    \item An~$n$-tuple~$\tau$ is a \emph{support} on a~$n$-ary constraint~$c$ if and only
      if~$\tau$ is valid for~$vars(c)$ and~$\tau$ is allowed by~$c$, that is,
...
%  Each variable~$v_i \in V$ has possible values defined by its respective domain
 % ~$D_i \in D$.
\end{definition}

% \begin{definition}
%   \textbf{COP.} A Constrained Optimisation Problem (COP) is a 
%   four-tuple~$\Tuple{V,D,C,f}$, where:
%   \begin{itemize}
%     \item The triple~$\Tuple{V,D,C}$ is a CSP, and
%     \item $f$ is a function from~$D_1 \times \ldots \times D_n$ to 
%   \end{itemize}
% \end{definition}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item   Let~$c$ be an $n$-ary constraint on~$V$. A store~$s$ is a \emph{solution store} 
      to~$c$ if and only if all variables are fixed under~$s$, and the
      corresponding~$n$-tuple is allowed by~$c$:
      $\forall i \in \Set{1,\ldots,n}, s(v_i) = \Set{a_i}$,
      and~$\left<a_1,\ldots,a_n\right>$ is allowed by~$c$.
...
In order to have well-defined behaviour of prfkljslkfjopagators, they must meet certain obligations.
...
\begin{definition}
  \textbf{Table constraints.} A (positive) \emph{table constraint c} is a
  constraint such that~$rel(c)$ is defined explicitly by listing all the
  tuples that are allowed by~$c$.
\end{definition}
