...
\begin{proof}[Proof of \Lemmaref{lemma:checking}]
  ...
  Now assume that~$s$ is a solution store, that is,~$\tau \in c$. In the
  case of initialisation, CT signals subsumption because all variables are 
  assigned, in line \Todo{which} in \T{initCT()}.
  If it is not initialisation, CT signals subsumption because all variables
  are assigned, in line \Todo{which} in \T{filterDomains()}.
...
After proving Lemmas~\ref{lemma:decreasing}-\ref{lemma:domain-consistent},
proving~\Thmref{thm:prop} is trivial.

\begin{proof}[Proof of \Thmref{thm:prop}]
  The theorem follows by Lemmas~\ref{lemma:decreasing}-
  \ref{lemma:domain-consistent}.
\end{proof}
