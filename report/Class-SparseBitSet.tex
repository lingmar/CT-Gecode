    \STATE $\Class$ CompressedSparseBitSet
    \item[]
      \STATE $\Words$: array of $64$-bit int \COMMENT{$\Words.\function{length}() = p$} \label{line:sbsfield:start}
      \STATE $\Index$: array of int \COMMENT{$\Index.\function{length}() = p$}
      \STATE $\Limit$: int 
      \STATE $\Mask$: array of $64$-bit int \COMMENT{$\Mask.\function{length}() = p$} \label{line:sbsfield:end}
    ...
    \item[]
      \Func{addToMask}{$\localvar{m}$: array of $64$-bit int} \label{line:addToMask:1}
    ...
    \item[]
      \FuncRet{intersectIndex}{$\localvar{m}$: array of $64$-bit int}{int} \label{line:interIdx:1}
